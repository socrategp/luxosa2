    ...
    <div className="fixed inset-x-0 bottom-0 z-[70] border-t border-sand/60 bg-ivory/95 backdrop-blur-md">
      <div className="mx-auto flex max-w-[1400px] flex-col gap-6 px-6 py-6 md:flex-row md:items-center md:justify-between md:px-10 lg:px-16">
        <div className="max-w-3xl">
          <span className="mb-3 block text-[10px] font-light uppercase tracking-[0.32em] text-brass-muted">{t('common:cookie.consent.banner.001')}</span>
          <p className="text-[14px] font-light leading-[1.8] text-anthracite/75 md:text-[15px]">{t('common:cookie.consent.banner.002')}</p>
        </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row md:flex-shrink-0">
          ...
          >{t('common:cookie.consent.banner.003')}</button>
          <button
            type="button"
          ...
          >{t('common:cookie.consent.banner.004')}</button>
        </div>
      </div>
    </div>
  );
