...
function QuizContent({
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-[960px] mx-auto px-4 sm:px-6 md:px-10 py-8 md:py-12 lg:py-14"
    >
      {/* Question header */}
...
function FormScreen({ onSubmit }: { onSubmit: (data: ContactFormData) => void }) {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -20 }}
      transition={{ duration: 0.5, ease: premiumEase }}
      className="w-full max-w-[580px] mx-auto px-6 md:px-10 py-10 md:py-14"
    >
...
function ResultScreen({
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-[1100px] mx-auto px-6 md:px-10 lg:px-12 py-10 md:py-14"
    >
    ...
    >
      ...
      <div className="flex-shrink-0 bg-ivory border-b border-sand/40 h-[64px] md:h-[80px] w-full flex items-center">
        <div className="w-full px-4 sm:px-6 md:px-10 flex items-center justify-between gap-4">
          ...
          {/* Left: back or logo */}
          <div className="w-20 md:w-32 flex items-center flex-shrink-0">
            {screen === 'quiz' && step > 0 ? (
              <button
                onClick={() => setStep(s => Math.max(0, s - 1))}
                className="flex items-center gap-2 text-[11px] tracking-[0.2em] uppercase text-charcoal/40 hover:text-charcoal transition-colors group outline-none"
              >
